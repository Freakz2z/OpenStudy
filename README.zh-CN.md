...
<p align="center"><em>一张真实流程 GIF：从导入 Markdown、查看源码、开始做题、问 AI、进入错题本重做，再到洞察复盘。</em></p>
...
上面的 GIF 展示的就是这条完整链路：`导入 Markdown -> 编辑源码 -> 开始做题 -> 问 AI -> 错题本 -> 重做 -> 洞察`。

## 标准

OpenStudy 虽然是 Markdown-first，但并不是“随便写都行”的 Markdown。我们约定了一套稳定的题目标准，这样 AI 清洗、结构化识别、做题、重做和复盘才能始终一致。
